...
public class Sudoku
{
    ...
    public static int[,]? CreateGameGrid(int[,] originalGrid, int numbersToErase){

        int[,] gameGrid = originalGrid.Clone() as int[,] ?? throw new ArgumentNullException("grid is null");
        Random rand = new Random();
        int toEraseCopy = numbersToErase;
        int outerZeros = 0;
        while (
            numbersToErase > 0
            &&
            toEraseCopy > gameGrid.Count2D(0)
            )
        {
            int row, col;
            do
            {
                row = rand.Next(0, 9);
                col = rand.Next(0, 9);
            } while (originalGrid[row, col] == 0);

            int backup = originalGrid[row, col];
            gameGrid[row, col] = 0;

            int[,] copyGrid = gameGrid.Clone() as int[,] ?? throw new ArgumentNullException("wrong casting");
            int innerCounter = 0;


            Solver(copyGrid,ref innerCounter);
            int zerosAfter = gameGrid.Count2D(0);
            //debug
            if (numbersToErase == 10)
            {
                _ = 420;
            }
            ...
            else
            {
                ...
                if(zerosAfter > outerZeros)
                    numbersToErase -= 1;
            }
            outerZeros = zerosAfter;
    ...
    private static void Solver(int[,] grid,ref int innerCounter)
    {
        for (int i = 0; i < 9*9; i++)
        {
            int row = i / 9;
            ...
            if (grid[row, col] == 0)
            {
                ...
                {
                    if (!grid.SliceRow(row).Any(x => x == value))
                    {
                        if (!grid.SliceColumn(col).Any(x => x == value))
                        {
                            int[] square = new int[9];
                            List<int> list = new List<int>();
                            int squareCounter = 0;
